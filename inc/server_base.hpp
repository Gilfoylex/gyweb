...
namespace gyweb{
	...
	public:
		...
		void start()
		{
			...
			for (auto it = default_resource.begin(); it != default_resource.end()l it++)
			{
				all_resources.push_back(it);
			}
	...
	protected:
		...
		std::std::vector<resource_type::iterator> all_resources;
		...
		void respond(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request) const
		{
				...
				for(auto res_it: all_resources)
				{
					...
					{
						if(res_it->second.count(request->methond) > 0)
						{
